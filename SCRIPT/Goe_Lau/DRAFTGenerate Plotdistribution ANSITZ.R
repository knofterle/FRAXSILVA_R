################ GENERATE PLOT DISTRIBUTION TABLE ANSITZ #######################
# J.Osewold
# 21.10.2021
##### DRAFT #####
################################################################################
...
###### LOAD DATABASE -----------------------------------------------------------
plot_coord_csv <- read.csv("DATA/RAW/Goe_Lau/Plot Distribution/Goe_Ansitz.csv",
                           header = T)
...
# the relative coordinates are shifted to put the reference plot (8710) at 0|0  
plot_coord_rel[,2:3] <- plot_coord_rel[,2:3]+2
...
zero_x <- reference$x [reference$nr == "ref8710"]
zero_y <- reference$y [reference$nr == "ref8710"]

####### ADJUST THESE VALUES ------------------------------------------------------
plotdistance <- -5 
plotangle_a <- 75.5 * pi/180
plotangle_b <- plotangle_a * 1.02 + pi/2
#######
...
check <- reference["ref8815",2:3] - plot_coord_abs[ "8815", 2:3]
sqrt( sum(check**2))

###### TIDY UP -----------------------------------------------------------------
nvplots_ansitz <- plot_coord_abs
ref_ansitz <- reference
